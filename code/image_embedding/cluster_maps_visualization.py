...
def percentage_histograms(tag_maps):
    ...
    for tag, tag_map in tag_maps.items():
        ...
        plt.savefig(curr_folder / vis_type / 'tag_percentage_histo' / (tag + '.png'))
        plt.clf()
...
def percentage_map(tag_maps):
    ...
    io.imsave(curr_folder / vis_type / 'tag_perc_map.png', color_map)
    ...
    for tag, percentage_map in percentage_maps.items():
        ...
        io.imsave(curr_folder / vis_type / 'tag_percentage_maps' / (tag + '.png'), tag_color_map)
...
def activation_map(tag_maps):
    ...
    for tag, percentage_map in percentage_maps.items():
        ...
        io.imsave(curr_folder / vis_type / 'tag_activation_maps' / (str(tag) + '.png'), activation_map)
        total_activation_map += activation_map
        activation_maps[tag] = activation_map

    io.imsave(curr_folder / vis_type / 'tag_activ_map.png', total_activation_map)
...
def topology_landscape(matrix, typename):
    ...
    plt.savefig(curr_folder / vis_type / (typename + '_plot.png'))
    plt.clf()
...
def sw_activation_map(tag_maps):
    ...
    io.imsave(curr_folder / vis_type / 'sw_activation_map.png', sw_activation_map)
...
input_dir = curr_folder / 'data'

random.seed(0)
tag_maps_location = Path(input_dir / (vis_type + '_maps.p'))
print(tag_maps_location)
...
image_grid_location = Path(input_dir / 'image_grid.p')
